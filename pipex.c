...
/*   Updated: 2026/02/09 13:38:10 by slambert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (pipe(pipex.fd) == -1)
		clean_exit("pipe returned -1", NULL, pipex.fd, 1);
	pipex.pid1 = fork();
	if (pipex.pid1 < 0)
		clean_exit("first fork failed", NULL, pipex.fd, 1);
	if (pipex.pid1 == 0)
		child_cmd_1(&pipex);
	pipex.pid2 = fork();
	if (pipex.pid2 < 0)
		clean_exit("second fork failed", NULL, pipex.fd, 1);
	if (pipex.pid2 == 0)
		child_cmd_2(&pipex);
	close(pipex.fd[0]);
	close(pipex.fd[1]);
...
void	child_cmd_1(t_pipex *pipex)
{
	...
	{
		if (errno == EISDIR)
			clean_exit("is a directory", pipex->argv[1], pipex->fd, -1);
		if (errno == EACCES)
			clean_exit("permission denied", pipex->argv[1], pipex->fd, -1);
		clean_exit("no such file or directory", pipex->argv[1], pipex->fd, -1);
	}
	if (dup2(pipex->fd_infile, STDIN_FILENO) < 0)
		clean_exit("dup2 infile error", NULL, pipex->fd, pipex->fd_infile);
	if (dup2(pipex->fd[1], STDOUT_FILENO) < 0)
		clean_exit("dup2 pipe error", NULL, pipex->fd, pipex->fd_infile);
...
void	child_cmd_2(t_pipex *pipex)
{
	...
	{
		if (errno == EISDIR)
			clean_exit("is a directory", pipex->argv[4], pipex->fd, -1);
		if (errno == EACCES)
			clean_exit("permission denied", pipex->argv[4], pipex->fd, -1);
		clean_exit("no such file or directory", pipex->argv[4], pipex->fd, -1);
	}
	if (dup2(pipex->fd[0], STDIN_FILENO) < 0)
		clean_exit("dup2 pipe error", NULL, pipex->fd, pipex->fd_outfile);
	if (dup2(pipex->fd_outfile, STDOUT_FILENO) < 0)
		clean_exit("dup2 outfile error", NULL, pipex->fd, pipex->fd_outfile);
...
// error message for 127 is "command not found: "
// or "no such file or directory: "
void	error_exit(char *error_msg, int status, int free_msg)
{
	if (status == 127)
		error_127(error_msg, status, free_msg);
	if (status == 126)
		error_126(error_msg, status, free_msg);
	perror(error_msg);
	if (free_msg)
		free(error_msg);
	exit(status);
